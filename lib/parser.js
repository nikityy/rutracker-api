  ...
  parseSearch(rawHtml) {
    ...
    for (let i = 0; i < length; i += 1) {
      ...
      const torrent = new Torrent({
        state: this.parseState(state.attr("title")),
    ...
    return results.filter(x => Boolean(x.id));
  }

  parseState(title) {
    const mapping = {
      'проверено': Torrent.APPROVED,
      'не проверено': Torrent.NOT_APPROVED,
      'недооформлено': Torrent.NEED_EDIT,
      'сомнительно': Torrent.DUBIOUSLY,
      'поглощено': Torrent.CONSUMED,
      'временная': Torrent.TEMPORARY,
    };
